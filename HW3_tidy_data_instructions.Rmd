...
This next table gets much more involved. You will need to coerce some strings into the specified forms given in the tidy table. 
...
\end{table}
```
You will need to trim the `reltrad` column of `pew` from:
...
For `tb` starter code will take care of some first-step, minor, tidy-ups. Next, use `gather` to construct the molten version, before doing some additional clean-up/ rearrangement to reach the tidy format.
...
A quick `gather()` and voila, you've made `melt_weather`.
